...
# The env file is the normal way in. Continuous integration has no such file
# and holds the same values as secrets in the environment instead, so a missing
# file is only an error when the environment is empty too. Whichever way they
# arrive, the checks below are the same.
if [ ! -f "$ENV_FILE" ]; then
  if [ -n "${CLOUDFLARE_API_TOKEN:-}" ] && [ -n "${CLOUDFLARE_ACCOUNT_ID:-}" ]; then
    ok "no $ENV_FILE, using the environment"
    ENV_FILE=""
  else
    die "$ENV_FILE not found. Copy ${ENV_FILE}.example to $ENV_FILE and fill it in, or set the values in the environment."
  fi
fi
...
while IFS= read -r line || [ -n "$line" ]; do
  ...
  value="${value%\'}"; value="${value#\'}"
  export "$key=$value"
done < <(if [ -n "$ENV_FILE" ]; then cat "$ENV_FILE"; fi)
[ -n "$ENV_FILE" ] && ok "read $ENV_FILE"

require() {
  local name="$1"
  local value="${!name:-}"
  [ -n "$value" ] || die "$name is empty${ENV_FILE:+ in $ENV_FILE}."
}
...
if [ "$PROFILE" = "demo" ] && [ -z "${DEMO_COOKIE_SECRET:-}" ]; then
  die "DEMO_COOKIE_SECRET is empty${ENV_FILE:+ in $ENV_FILE}."
fi
...
if [ -n "${CLOUDFLARE_API_TOKEN:-}" ]; then
  ...
  ok "using the configured API token"
elif [ "$DRY_RUN" = true ]; then
  skip "no CLOUDFLARE_API_TOKEN set, a real run would open a browser to sign in"
elif [ "$INTERACTIVE" = false ]; then
  die "No API token and not interactive. Set CLOUDFLARE_API_TOKEN."
else
...
{
  echo "# Generated by scripts/deploy.sh${ENV_FILE:+ from $ENV_FILE}. Do not edit or commit."
