...
- **Status:** [!] BLOCKED (partial — see Worklog)
...
| TICKET-31 | 2026-07-04 | `src/lib/parsers/`: `parseText` (UTF-8 decode, latin1 fallback), `parseCsv` (papaparse → GFM table; >500 rows splits into `##`-sectioned tables of ≤200 rows, each with its own header/separator so the chunker never has to split a table), `parseDocx` (mammoth `convertToMarkdown` — not in mammoth's shipped `.d.ts`, called via a local typed cast; passes both `buffer` and `arrayBuffer` since mammoth's Node build reads the former and its "browser" build, which Wrangler's bundler substitutes for Worker code, reads the latter — verified both paths work via a temporary spike route, removed before commit), `extractPdfText` (`unpdf`, returns `{ text, totalPages, charsPerPage }`). Both `mammoth` and `unpdf` ran correctly under `wrangler dev --local` on the first try with the `nodejs_compat` flag already enabled from TICKET-07 — no additional compat work needed. Regenerated `fixtures/sample.pdf` and added `fixtures/{sample.txt,sample.csv,sample.docx,digital.pdf,scanned.pdf}` — all hand-built with a correct-offset PDF builder / minimal valid OOXML zip (real word processors weren't available in this environment) — and confirmed `digital.pdf` averages 277 chars/page (routes local) vs. `scanned.pdf`'s 0 (routes LlamaParse), validating the fixtures before writing tests against them. Verified: `npx vitest run` — 9 tests, all green; typecheck/lint green. | apps/api/src/lib/parsers/text.ts, apps/api/src/lib/parsers/csv.ts, apps/api/src/lib/parsers/docx.ts, apps/api/src/lib/parsers/pdf.ts, apps/api/src/lib/parsers/errors.ts, apps/api/src/lib/parsers/index.ts, apps/api/src/lib/parsers/test-utils.ts, apps/api/src/lib/parsers/*.test.ts, apps/api/vitest.config.ts, apps/api/fixtures/sample.pdf, apps/api/fixtures/sample.txt, apps/api/fixtures/sample.csv, apps/api/fixtures/sample.docx, apps/api/fixtures/digital.pdf, apps/api/fixtures/scanned.pdf, apps/api/package.json |
| TICKET-32 | 2026-07-04 | **BLOCKED (partial).** Implemented `lib/router.ts`'s `routeDocument(env, doc, getBuf)` in full: exported `TRIAGE` constants object (`MAX_LOCAL_BYTES=20MB`, `MIN_CHARS_PER_PAGE=200`, `MIN_NON_EMPTY_PAGE_RATIO=0.8`, `MAX_GARBAGE_RATIO=0.05`), the size guard, `.txt`/`.md`/`.csv` always-local rule, `.docx` try-local-then-`UnsupportedLocalParse`-fallback, and the PDF heuristic (avg chars/page + non-empty-page ratio + replacement-char garbage ratio). `getBuf` is a lazy `() => Promise<ArrayBuffer>` rather than a raw buffer specifically so the truly-oversized PDF/DOCX fallback path never materializes a buffer it won't use. Local routing sets `parser='local'`, `status='embedding'`; LlamaParse routing sets `parser='llamaparse'`, `status='parsing'`, and upserts `usage_daily` (`api='llamaparse'`, `tokens=totalPages` via `onConflictDoUpdate`) — verified the upsert increments correctly across repeated runs. Wired into `POST /api/documents` (materializes `file.arrayBuffer()` only after the R2 stream upload, since Files/Blobs are independently re-readable) and tightened `UploadMeta`'s zod schema so `.txt/.csv/.md` over 20 MB are rejected at upload (they can never fall back to LlamaParse, so oversized ones have nowhere to go). **Why blocked:** the ticket's own text says LlamaParse routing should "proceed to TICKET-11 submission," and TICKET-11's `lib/llamaparse.ts` doesn't exist yet — despite the dependency graph listing only TICKET-31 as this ticket's dependency, TICKET-32's literal acceptance criteria ("both reach `active`", "a job id exists") requires functionality from TICKET-13/14 (chunker/embedder — status can't reach `active` without them; explicitly out of scope for this ticket to build) and TICKET-11 (real LlamaParse job id — blocked on the missing `LLAMA_CLOUD_API_KEY`, per TICKET-11's own instructions not to fake it). Recorded as D14/D15. What WAS verified end-to-end against `wrangler dev --local` via extended `upload-smoke.sh`: `sample.csv` and `digital.pdf` → `parser='local'`, `status='embedding'`; `scanned.pdf` → `parser='llamaparse'`, `status='parsing'`; oversized `.txt` rejected at upload; `usage_daily` accrues correctly — ran twice, consistent both times. `npm run typecheck`/`lint`/`vitest run` all still green (no regressions). | apps/api/src/lib/router.ts, apps/api/src/routes/documents.ts, apps/api/scripts/upload-smoke.sh |
| TICKET-11 | 2026-07-04 | **BLOCKED — no work performed beyond checking for credentials.** Checked the sanctioned local-secrets location (`apps/api/.dev.vars`, per §2.3) for `LLAMA_CLOUD_API_KEY`: not present. No such key is available anywhere in this environment. Per this ticket's own acceptance criteria ("If no key available, mark `[!] BLOCKED`... do NOT stub fake success") and §0 rule 6, did not write speculative `lib/llamaparse.ts` client code against the LlamaCloud REST API — the ticket itself requires consulting *current* LlamaCloud docs "at implementation time," which isn't something to guess at without a real account to verify request/response shapes against; fabricating unverified endpoint paths would violate §0 rule 6's "do not guess." Stopping the autonomous ticket run here, per the operator's instruction to work until hitting a missing-credential requirement. To unblock: add `LLAMA_CLOUD_API_KEY` to `apps/api/.dev.vars` and re-run this ticket. | _(none — blocked before implementation)_ |
| TICKET-23 | 2026-07-04 | `POST /api/admin/provision` (`src/routes/admin.ts`), guarded by a `Bearer <ADMIN_TOKEN>` header check (new secret, generated locally into `.dev.vars`, added to `Env`). Given `{ email, orgName }`: calls `auth.api.signUpEmail` with a random throwaway password (the account is only ever reached via the reset link, never signed into with this password) — this reuses the existing `session.create.before` org-auto-provisioning hook from TICKET-07 rather than duplicating it, then renames the auto-created organization to the requested `orgName` via a direct `UPDATE`. Calls `auth.api.requestPasswordReset` to generate a one-time reset link; extended `createAuth(env, hooks)` in `src/lib/auth.ts` with an optional `onSendResetPassword` hook so the route can capture the link directly in its response instead of it being emailed (no email provider exists in this stack, and none is planned in any ticket) — confirmed `runInBackgroundOrAwait` (better-auth internals) actually awaits `sendResetPassword` synchronously when no `advanced.backgroundTasks.handler` is configured, so there's no race capturing the value. `apps/api/scripts/provision-tenant.ts` is a thin Node CLI wrapper (`node scripts/provision-tenant.ts <email> <orgName> [baseUrl]`) — runs directly via Node 22's default TypeScript stripping, no build step. Added a focused "Tenant onboarding" section to a new root `README.md` per this ticket's own text (the comprehensive README is TICKET-26). Verified end-to-end against `wrangler dev --local`: unauthenticated/wrong-token requests get 401; running the script provisions a tenant, and manually completing the returned reset link (`POST /api/auth/reset-password`) then signing in with the new password returns a session whose `/api/me` shows the correct `tenantId`, and the organization's `name` in D1 matches the requested `orgName` — i.e., a genuinely login-able tenant, per the acceptance criteria. typecheck/lint/vitest all green. | apps/api/src/routes/admin.ts, apps/api/src/lib/auth.ts, apps/api/src/env.ts, apps/api/src/index.ts, apps/api/scripts/provision-tenant.ts, README.md |
| TICKET-26 | 2026-07-04 | Expanded `README.md` into the full runbook: local dev setup (clone → install → `.dev.vars` → migrations → dev servers → smoke scripts → typecheck/lint/vitest), the full §2.3 env var table (bindings + secrets + vars, including the not-yet-wired `LLAMA_CLOUD_API_KEY`/`OPENAI_API_KEY`/`DEEPSEEK_API_KEY` needed by later tickets), migration commands (drizzle-kit generate, wrangler d1 migrations apply --local/--remote, better-auth CLI regeneration), the existing tenant-onboarding section, forward-looking deploy steps (marked as "TICKET-28 executes this"), and free-tier limits stated as "verify at deploy time" with links to the relevant Cloudflare docs pages rather than hardcoded numbers, per the ticket's own instruction. Verified per the ticket's literal acceptance test: cloned the repo into a fresh directory and followed only the README — `npm install`, `.dev.vars` creation, `wrangler d1 migrations apply --local`, `npm run dev:api` + `npm run dev:web`, both smoke scripts, the provisioning script, and `typecheck`/`lint`/`vitest run` all worked exactly as documented with no deviation needed. | README.md |
...
| D14 | 2026-07-04 | TICKET-32 marked `[!] BLOCKED (partial)` rather than `[x] DONE`, despite implementing and verifying its full router logic | Its literal acceptance criteria ("both reach `active`", "a job id exists") depends on TICKET-13/14 (chunker/embedder — not yet built, and explicitly out of scope for TICKET-32 itself) and TICKET-11 (real LlamaParse submission — needs `LLAMA_CLOUD_API_KEY`, unavailable). The dependency graph (§8) only lists TICKET-31 as TICKET-32's dependency, but TICKET-32's own text ("LlamaParse route: proceed to TICKET-11 submission") and TICKET-11's text ("Invoked ONLY by the TICKET-32 router's fallback branch") describe a forward reference the graph doesn't capture. Future sessions: this ticket's code is complete and correct for what's currently buildable (verified via `upload-smoke.sh` against real `wrangler dev --local` state); only re-run its acceptance test once TICKET-11/13/14 exist, don't re-implement it |
| D15 | 2026-07-04 | `.txt`/`.csv`/`.md` uploads are rejected outright above `TRIAGE.MAX_LOCAL_BYTES` (20 MB), tightening TICKET-10's original 100 MB ceiling for just these three extensions | TICKET-32 rule 2 forbids ever routing these to LlamaParse, and local parsing needs the whole buffer resident — so an oversized one has no safe path through the pipeline at all. Ticket text named only `.txt`/`.csv` explicitly; `.md` is included too since it's handled identically to `.txt` everywhere else in the local-parser tier |
| D16 | 2026-07-04 | Admin-provisioned tenants (TICKET-23) reuse the existing `session.create.before` org-auto-provisioning hook (via `auth.api.signUpEmail`) and then rename the resulting org, rather than inserting `organization`/`member` rows directly in the admin route | Avoids a second, slightly-different copy of "create an org for this user" logic living in two places; the rename is a one-line `UPDATE` and keeps org-creation semantics (slug format, owner role, etc.) in exactly one place (`src/lib/auth.ts`) |
| D17 | 2026-07-04 | No email provider is configured or planned; TICKET-23's "one-time password-reset/invite link" is returned directly in the `/api/admin/provision` JSON response, not emailed | Confirmed via reading better-auth's compiled source (`api/routes/password.mjs`, `context/create-context.mjs`) that `sendResetPassword` is `await`-ed synchronously (not fire-and-forget via `waitUntil`) as long as `advanced.backgroundTasks.handler` isn't configured — so capturing the token/url through an injected callback (`createAuth`'s new `onSendResetPassword` hook) is reliable, not a race, and needs no email infrastructure |
